...
class Check @JvmOverloads constructor(
    ...
    init {
        ...
        text = a.getText(R.styleable.Check_android_text)
        outlinePaint = Paint(ANTI_ALIAS_FLAG).apply {
            color = a.getColorOrThrow(R.styleable.Check_android_strokeColor)
            strokeWidth = a.getDimensionOrThrow(R.styleable.Check_outlineWidth)
            style = STROKE
        }
        defaultTextColor = a.getColorOrThrow(R.styleable.Check_android_textColor)
        textPaint = TextPaint(ANTI_ALIAS_FLAG).apply {
            color = defaultTextColor
            textSize = a.getDimensionOrThrow(R.styleable.Check_android_textSize)
        }
        dotPaint = Paint(ANTI_ALIAS_FLAG)
        clear = a.getDrawableOrThrow(R.styleable.Check_clearIcon).apply {
            setBounds(
                    -intrinsicWidth / 2, -intrinsicHeight / 2, intrinsicWidth / 2, intrinsicHeight / 2
            )
        }
        touchFeedback = a.getDrawableOrThrow(R.styleable.Check_foreground).apply {
            callback = this@Check
        }
        padding = a.getDimensionPixelSizeOrThrow(R.styleable.Check_android_padding)
        isChecked = a.getBoolean(R.styleable.Check_android_checked, false)
        showIcons = a.getBoolean(R.styleable.Check_showIcons, true)
        a.recycle()
        clipToOutline = true
    }
